 ...
 */
 ...
 public class TicTacToeFinal
 {
	 ...
	 */
 	 ...
 	 * error handling for this). Then, check if there is a winner by passing it into the 
 	 * checkWinner function, and switch players. Print the final board once someone has
 	 * won/tied. Based on the status, print congrats/lost/tie
     ...
     *Goes through all possible combinations of winning the tic tac toe game by calling 
     *those three methods
     *@param symbol Player’s symbol
     *@return whether or not the player has won either horizontally, diagnoally, or 
     *vertically (through a boolean value)
     */
 	...
 	public static boolean symbolWin(String symbol)
 	{
		...
		if (verticalCheck(symbol))
		{
			return true;
		}
		if (diagonalCheck(symbol))
		{
			return true;
		}
 	...
 	/**
	*Checks to see if the same symbol is in every space of a particular row (if so, true is
	*returned)
	*Algorithm: Goes through every column in a specific row to check whether the item within 
	*that space is the same as the parameter (that being the symbol)
	*If at any point a given space within a row is not the same as the given symbol, false 
	*will be returned. Otherwise, true will be returned as every space within a given row
	*is the same as the given symbol
	*@param symbol This is the symbol that is being checked against with every space in a 
	*given row
	*@return whether or not a single space in a given row is the same symbol as the other 
	*spaces
	*/	
 	...
 	/**
	*Checks to see if the same symbol is in every space of a particular column (if so, true 
	*is returned)
	*Algorithm: Goes through every row in a specific column to check whether the item within 
	*that space is the same as the parameter (that being the symbol)
	*If at any point a given space within a column is not the same as the given symbol, 
	*false will be returned. Otherwise, true will be returned as every space within a given
	*column is the same as the given symbol
	*@param symbol This is the symbol that is being checked against with every space in a 
	*given column
	*@return true whether or not a single space in a given column is not the same symbol 
	*as the other spaces
	*/	
